...
import { SCENE_ADMIN_SEND_MESSAGE_REGULAR } from "../../../app/constants/constants.scene";
import { SendMessageServices } from "../../../services/SendMessageServices";
import { createInlineKeyboard } from "../../../utils";
import { ADMIN_SEND_MESSAGE_REGULAR_ROUTE } from "../../../configs/routes";
import { googleReportService } from "../../../services/Google/GoogleReportService";

export class AdminSendMessageRegularScenes {
...
Напишите любое сообщение и отправьте его — оно будет разослано всем постоянным пользователям.`);
      ...
      async (ctx) => {
        ...
        if (ctx.callbackQuery) {
          ...
          if (ctx.callbackQuery.data === "send_mailing") {
            ...
            try {
              const listUsers = await googleReportService.getAllRegularUserId();
